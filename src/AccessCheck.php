...
class AccessCheck implements AccessCheckInterface{
      ...
      if ($field->getFieldDefinition()->getType() == 'entity_reference' && $field->getFieldDefinition()->getSetting('target_type') == 'taxonomy_term') {
        ...
        if (!empty($aReferencedTaxonomyTerms)) {
          foreach ($aReferencedTaxonomyTerms as $aReferencedTerm) {
            if (isset($aReferencedTerm['target_id']) && !$this->isAccessAllowedByDatabase($aReferencedTerm['target_id'], $uid, $node->bundle())) {
   ...
   * {@inheritdoc}
   */
  public function isAccessAllowedByDatabase($tid, $uid = FALSE, $contentType) {
    ...
    foreach ($aUserRoles as $sUserRole) {
      ...
      if ($this->isTermAllowedByUserRoleForContentType($tid, $sUserRole, $contentType)) {
        return TRUE;
      }
...
*/
 ...
 /* public function isTermAllowedByUserId($tid, $iUid) {
    ...
    $query_result = db_query("SELECT uid FROM {permissions_by_term_user_content_type} WHERE tid = :tid AND uid = :uid",
...
*/
   ...
   * {@inheritdoc}
   */
  public function isTermAllowedByUserRoleForContentType($tid, $sUserRole, $contentType) {
    $query_result = db_query("SELECT rid FROM {permissions_by_term_role_content_type} WHERE tid = :tid AND rid IN (:user_roles) AND content_type = :ct",
      array(':tid' => $tid, ':user_roles' => $sUserRole, ':ct' => $contentType))->fetchField();
  ...
  public function isAnyPermissionSetForTerm($tid) {
    ...
    /*$iUserTableResults = intval(db_query("SELECT COUNT(1) FROM {permissions_by_term_user_content_type} WHERE tid = :tid",
      array(':tid' => $tid))->fetchField());
*/
    $iRoleTableResults = intval(db_query("SELECT COUNT(1) FROM {permissions_by_term_role_content_type} WHERE tid = :tid",
      array(':tid' => $tid))->fetchField());

    //if ($iUserTableResults > 0 ||
      if ($iRoleTableResults > 0) {
      return TRUE;
    }
