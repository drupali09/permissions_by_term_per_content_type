...
use Drupal\permissions_by_term_per_content_type\Factory\NodeAccessRecordFactory;
...
class NodeAccess {
  ...
  public function createRealm($uid) {
    return 'permissions_by_term_per_content_type__uid_' . $uid;
  }
  ...
  public function rebuildByTid($tid, $formState) {
    ...
    if (empty($this->accessStorage->getSubmittedRolesGrantedAccess($formState))) {
      return;
    }
  ...
  public function rebuildByTidForContentType($tid) {
    $nids = $this->accessStorage->getNidsByTid($tid);
    if (!empty($nids)) {
      $this->dropRecordsByNids($nids);
    }
  ...
  private function dropRecordsByUid($uid) {
    $this->database->delete('node_access')
      ->condition('realm', 'permissions_by_term_per_content_type__uid_' . $uid)
      ->execute();
  }
