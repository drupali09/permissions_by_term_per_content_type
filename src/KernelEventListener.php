 ...
 *
 * @package Drupal\permissions_by_term_per_content_type
 */
class KernelEventListener implements EventSubscriberInterface {
  ...
  public function __construct() {
    $this->accessCheckService = \Drupal::service('permissions_by_term_per_content_type.access_check');
  ...
  public function onKernelRequest(GetResponseEvent $event) {
    ...
    if ($event->getRequest()->attributes->get('target_type') == 'taxonomy_term' &&
      ...
      $contentType = $event->getRequest()->attributes->get('node_type')->get('type');
      $tid = $this->accessStorageService->getTermIdByName($query_string);
      if (!$this->accessCheckService->isAccessAllowedByDatabase($tid, $uid = FALSE, $contentType)) {
        $this->sendUserToAccessDeniedPage();
      }
    }
  }
